...
HTTP response:
  authenticate(registration):
    ...
    Error:
      ...
      status 402: no token recieved, return "Token absent"
  ...
  verify_email:
    OK:
      status 200: verified
    Error:
      status 405: userID absent in header, return "Missing userID"
      status 405: userID not found in DB, return "invalid userID"
*/
  ...
  verify_email: async function verify_email(req, res, next) {
    if (req.query.id) {
      ...
      if (user.exists) {
        ...
        return res.status(406).send("invalid userID")
      }

    } else {
      return res.status(405).send("Missing userID")
    }
  }
